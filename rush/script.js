...
var interval = null;
let fromType, toType, currentQuestion, n;
let running = false;
...
document.getElementById("answer").addEventListener("keyup", (e) => {
    ...
    if (running) {
        if (document.getElementById("answer").value.toLowerCase() == currentQuestion[3].toString().toLowerCase()) {
            fromType = types[randi(0,2)];
            toType = types[randi(0,2)];
            while (toType == fromType) {
                toType = types[randi(0,2)];
            }
            n = randi(1,118);
            currentQuestion = [fromType, toType, data.elements[n][fromType], data.elements[n][toType]];
            document.getElementById("question").innerHTML = "What is the (atomic) " + currentQuestion[1] + " of element " + currentQuestion[2] + "?";
        ...
        interval = setInterval(() => {
            ...
            if ((60.0 - elapsedTime / 1000) <= 0.0) {
                document.getElementById("timer").innerHTML = "0.000";
                running = false;
                clearInterval(interval);
            } else {
                document.getElementById("timer").innerHTML = (60.0 - elapsedTime / 1000).toFixed(3);
            }
        }, 7);
    }
})